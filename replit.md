...
- **Automatic People Tracking System**: ZEKE autonomously extracts and tracks relationships from Limitless lifelogs and conversations. Key components:
  - **Background Processor**: `peopleProcessor.ts` runs every 4 hours to scan recent lifelogs and auto-create contacts for identified people.
  - **Enhanced Contacts Schema**: Contacts now include: birthday, occupation, organization, email, lastInteractionAt, interactionCount, metadata (JSON), and isAutoCreated flag.
  - **Memory-to-Person Linking**: Memory notes can be linked to specific contacts via contactId, with sourceType (lifelog, conversation, manual) and sourceId tracking.
  - **Auto-Created Contacts**: Contacts extracted from lifelogs use unique phone placeholders (`auto-{uuid}`) to avoid collisions with real phone numbers while maintaining database integrity.
  - **People Tracking Tools**: 13 AI tools including: `auto_create_person`, `link_memory_to_person`, `update_person_info`, `get_person_memories`, `search_person_history`, `extract_people_from_lifelogs`, `get_recently_interacted_people`, `get_most_interacted_people`, `get_auto_created_contacts`, `find_person`, `list_people`, `update_contact`, `search_contacts`.
  - **Autonomous Permission**: ZEKE has full permission to auto-create contacts, update relationship info, and link memories without user confirmation. This enables proactive relationship management and context-rich conversations.
