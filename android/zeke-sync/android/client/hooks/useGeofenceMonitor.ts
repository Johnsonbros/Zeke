...
export function useGeofenceMonitor(enabled: boolean): UseGeofenceMonitorResult {
  ...
  const handleGeofenceTrigger = useCallback(async (
    geofence: Geofence,
    event: 'enter' | 'exit',
    latitude: number,
    longitude: number
    ...
    if (state?.lastTriggeredAt && now - state.lastTriggeredAt < TRIGGER_COOLDOWN_MS) {
      return;
    }
  ...
  const checkGeofences = useCallback(async (
    ...
    try {
      ...
      const nearby = findNearbyGeofences(userLocation, activeGeofences, NEARBY_DISTANCE_METERS);
      ...
      for (const geofence of activeGeofences) {
        ...
        if (!wasInside && isInside) {
          if (geofence.triggerOn === 'enter' || geofence.triggerOn === 'both') {
            await handleGeofenceTrigger(geofence, 'enter', latitude, longitude);
          } else {
            geofenceStateRef.current[geofence.id] = {
              isInside: true,
              lastTriggeredAt: geofenceStateRef.current[geofence.id]?.lastTriggeredAt || null,
            };
          }
        } else if (wasInside && !isInside) {
          if (geofence.triggerOn === 'exit' || geofence.triggerOn === 'both') {
            await handleGeofenceTrigger(geofence, 'exit', latitude, longitude);
          } else {
            geofenceStateRef.current[geofence.id] = {
              isInside: false,
              lastTriggeredAt: geofenceStateRef.current[geofence.id]?.lastTriggeredAt || null,
