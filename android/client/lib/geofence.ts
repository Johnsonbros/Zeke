...
export function calculateDistanceToGeofence(userLocation: UserLocation, geofence: Geofence): number {
  const R = 6371e3;
  const phi1 = (userLocation.latitude * Math.PI) / 180;
  const phi2 = (geofence.latitude * Math.PI) / 180;
  const deltaPhi = ((geofence.latitude - userLocation.latitude) * Math.PI) / 180;
  const deltaLambda = ((geofence.longitude - userLocation.longitude) * Math.PI) / 180;
...
export function isInsideGeofence(userLocation: UserLocation, geofence: Geofence): boolean {
...
export function findNearbyGeofences(
  ...
  const nearby: Array<{ geofence: Geofence; distance: number }> = [];
