...
import { ListsRepository, SyncService, getSyncStatus, type ListData, type ListItemData } from '@/lib/filesystem-repository';
...
export function useLocalLists() {
  ...
  const createList = useCallback(async (name: string, description?: string, color?: string) => {
    try {
      const newList = await ListsRepository.createList(name, description, color);
      await refetch();
      return newList;
    } catch (err) {
      throw err;
  ...
  const getListWithItems = useCallback(async (id: string): Promise<ListWithItems | null> => {
    try {
...
export function useLocalGrocery() {
  ...
  const addItem = useCallback(async (name: string, quantity?: number, unit?: string, category?: string) => {
    try {
      const newItem = await GroceryRepository.addItem(name, quantity, unit, category);
      await refetch();
      return newItem;
    } catch (err) {
      throw err;
